/**
 * Subtitle Parser and Format Converter
 * Supports: SRT, VTT, ASS/SSA, SUB, JSON, XML
 */
...
const SubtitleParser = {
  ...
  parseSUB(content) {
    ...
    lines.forEach((line, index) => {
      ...
      // Assume 25 fps for frame to ms conversion
      const fps = 25;
  ...
  /**
   * Parse JSON format (generic structure)
   */
  parseJSON(content) {
    try {
  ...
  parseXML(content) {
    try {
      ...
      // Try common XML subtitle formats
...
`;
  ...
  /**
   * Convert subtitles to SUB format
   */
  toSUB(subtitles) {
    const fps = 25;
    return subtitles.map(sub => {
...
<tt xmlns="http://www.w3.org/ns/ttml" xmlns:tts="http://www.w3.org/ns/ttml#styling" xml:lang="en">
  <head>
    <styling>
      <style xml:id="default" tts:fontFamily="Arial" tts:fontSize="16px" tts:textAlign="center" tts:color="white"/>
    </styling>
    <layout>
      <region xml:id="bottom" tts:extent="100% 20%" tts:origin="0% 80%" tts:textAlign="center" tts:displayAlign="after"/>
    </layout>
  </head>
  <body>
    <div region="bottom">`;
    ...
    const events = subtitles.map(sub => {
      const start = this.msToTTMLTime(sub.startMs);
      const end = this.msToTTMLTime(sub.endMs);
      return `      <p begin="${start}" end="${end}">${this.escapeXML(sub.text)}</p>`;
    }).join('\n');
  ...
  /**
   * Convert subtitles to SAMI format
   */
  toSAMI(subtitles) {
    const header = `<SAMI>
<HEAD>
...
<P Class="ENCC">${this.escapeHTML(sub.text)}</P>
  ...
  /**
   * Convert subtitles to SBV format (YouTube)
   */
  toSBV(subtitles) {
    return subtitles.map(sub => {
  ...
  stripASSFormatting(text) {
    // Remove ASS tags like {\c&H00FF00&} or {\i1}
    return text.replace(/\{[^}]+\}/g, '');
  },

  stripSubFormatting(text) {
    // Remove MicroDVD formatting like |C|text
    return text.replace(/\|[A-Z]\|/g, '');
  },

  /**
   * Parse TTML/DFXP format (used by Netflix, Apple, etc)
   */
  parseTTML(content) {
    try {
      const parser = new DOMParser();
      ...
      if (xmlDoc.parsererror) {
        console.error('TTML parsing error');
        return [];
      }

      const subtitles = [];

      // Find all span or p elements with timing info
      const elements = xmlDoc.querySelectorAll('[begin][end]');
      if (elements.length === 0) {
        // Try alternate structure
        xmlDoc.querySelectorAll('div p').forEach((p, index) => {
          const begin = p.getAttribute('begin') || p.getAttribute('start');
          const end = p.getAttribute('end');
          const text = p.textContent || '';

          if (begin && end && text.trim()) {
            subtitles.push({
              index: index + 1,
              startMs: this.ttmlTimeToMs(begin),
              endMs: this.ttmlTimeToMs(end),
              text: text.trim(),
              startTime: this.msToTime(this.ttmlTimeToMs(begin)),
              endTime: this.msToTime(this.ttmlTimeToMs(end))
            });
          }
        });
      } else {
        elements.forEach((elem, index) => {
          const begin = elem.getAttribute('begin');
          const end = elem.getAttribute('end');
  ...
  /**
   * Convert TTML time format to milliseconds
   * Supports: hh:mm:ss.mmm, hh:mm:ss, mm:ss.mmm, etc
   */
  ttmlTimeToMs(timeStr) {
    if (!timeStr) return 0;

    // Handle format like "00:00:05.000"
    const timeMatch = timeStr.match(/(\d+):(\d{2}):(\d{2})(?:\.(\d{3}))?/);
    if (timeMatch) {
      const hours = parseInt(timeMatch[1]);
      const minutes = parseInt(timeMatch[2]);
      const seconds = parseInt(timeMatch[3]);
      const milliseconds = parseInt(timeMatch[4] || 0);
      return hours * 3600000 + minutes * 60000 + seconds * 1000 + milliseconds;
    }

    // Handle format like "5000ms"
    const msMatch = timeStr.match(/(\d+)ms/);
    if (msMatch) {
      return parseInt(msMatch[1]);
    }

    // Handle format like "5s"
  ...
  /**
   * Parse SAMI format (Synchronized Accessible Media Interchange)
   */
  parseSAMI(content) {
    try {
      const parser = new DOMParser();
