 ...
 * Note: Safari uses 'browser' instead of 'chrome' API
 */

// Compatibility layer for Safari
...
<tt xmlns="http://www.w3.org/ns/ttml" xmlns:tts="http://www.w3.org/ns/ttml#styling" xml:lang="en">
  <head>
    <styling>
      <style xml:id="s1" tts:fontFamily="Arial" tts:fontSize="16px" tts:color="white"/>
    </styling>
  </head>
  <body>
    <div>`;
    ...
    subtitles.forEach(sub => {
      ...
      ttml += `
      <p begin="${start}" end="${end}" style="s1">${this.escapeHTML(sub.text)}</p>`;
    });
    ...
    try {
      ...
      pElements.forEach(p => {
        ...
        if (begin && end && text.trim()) {
          index++;
          const startMs = this.ttmlTimeToMs(begin);
          const endMs = this.ttmlTimeToMs(end);
  ...
  ttmlTimeToMs(timeStr) {
    const match = timeStr.match(/(\d+):(\d{2}):(\d{2})\.?(\d{0,3})/);
    if (!match) return 0;
